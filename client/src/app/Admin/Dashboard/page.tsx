...
import { FaUsers, FaBriefcase, FaSignOutAlt, FaChartBar, FaCog, FaUserShield } from 'react-icons/fa';
    ...
    <div className="flex min-h-screen bg-gray-50 text-[#121212]">
      ...
      <aside className="w-64 bg-white border-r border-gray-100 flex-col hidden md:flex">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <SidebarItem icon={<FaChartBar />} label="Overview" active />
          <SidebarItem icon={<FaUsers />} label="Manage Users" />
          <SidebarItem icon={<FaBriefcase />} label="Manage Jobs" />
      ...
      <main className="flex-1 p-8">
        <header className="mb-10 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-[#121212]">Dashboard Overview</h1>
...
const SidebarItem = ({ icon, label, active }: any) => (
  <button className={`w-full flex items-center gap-3 p-3.5 rounded-xl transition-all duration-200 font-medium ${active ? 'bg-[#0F172A] text-white shadow-lg shadow-slate-900/20' : 'text-gray-600 hover:bg-gray-50 hover:text-[#0F172A]'}`}>
