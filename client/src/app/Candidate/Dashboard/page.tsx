...
const CandidateDashboard = () => {
  ...
  const { data: freshUserData, isLoading: isLoadingFreshUser } =
    useGetCompanyByIdQuery(user?._id, {
      skip: !user?._id,
      pollingInterval: 30000, // Poll for updates every 30 seconds
    });
  const [markNotificationsAsRead] = useMarkNotificationsAsReadMutation();

  const { data: unreadChatData } = useGetUnreadMessageCountQuery(user?._id, {
    skip: !user?._id,
    pollingInterval: 5000,
  });
  ...
  const filteredJobs = sortedJobs.filter((job: any) => {
    ...
    return (
      matchesSearch && matchesIndustry && matchesWorkMode && matchesExperience
    );
  });
  ...
  const interviewsCount = appliedJobs.filter((job: any) => {
    const detail = job.applicantDetails?.find(
      (d: any) => d.candidateId === user?._id,
    );
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans text-[#121212]">
      ...
      >
        ...
        >
          <FaChevronLeft
            className={`text-[10px] text-[#0B0C10] transition-transform duration-300 ${isSidebarCollapsed ? "rotate-180" : ""}`}
          />
        </button>

        <div
          className={`p-6 h-20 flex items-center transition-all duration-300 ${isSidebarCollapsed ? "md:justify-center md:px-4" : ""}`}
        >
          <div
            className={`relative w-40 h-full z-50 transition-all duration-300 ${isSidebarCollapsed ? "md:hidden" : "block"}`}
          >
            <img
              src="/Click4Jobs Logo.png"
              alt="Click4Jobs"
              className="absolute top-1/2 left-0 -translate-y-1/2 h-[220px] w-auto max-w-none object-contain"
            />
          </div>
          <div
            className={`flex-shrink-0 hidden transition-all duration-300 ${isSidebarCollapsed ? "md:flex" : ""}`}
          >
            <img
              src="/Fav Icon.png"
              alt="Click4Jobs Icon"
        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto py-4 custom-scrollbar">
          ...
          <SidebarItem
            ...
            badge={
              appliedJobs.length > 0 ? appliedJobs.length.toString() : undefined
            }
            collapsed={isSidebarCollapsed}
          />
          <SidebarItem
          ...
          <SidebarItem
            ...
            badge={
              unreadMessageCount > 0 ? unreadMessageCount.toString() : undefined
            }
            collapsed={isSidebarCollapsed}
          />
        ...
        <header className="sticky top-0 z-30 bg-gray-50/90 backdrop-blur-md px-6 py-4 lg:px-10 border-b border-gray-200/50 transition-all duration-300">
          <div className="flex flex-wrap md:flex-nowrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-2xl font-bold text-[#121212] capitalize whitespace-nowrap">
                {activeTab.replace("-", " ")}
              </h1>
            </div>
            ...
            <div className="flex-1 w-full md:w-auto order-last md:order-none max-w-2xl mx-auto md:px-6 relative">
              <div className="relative group flex items-center">
                ...
                <button
                  ...
                  title="Filter Jobs"
                  className="absolute right-2 p-1.5 text-gray-400 hover:text-[#0B0C10] hover:bg-gray-50 rounded-xl transition-all"
                >
                  <FaFilter />
                </button>
              </div>
                  ...
                  <div className="absolute top-full mt-2 right-0 md:right-6 w-full md:w-80 bg-white rounded-2xl shadow-xl border border-gray-100 p-5 z-50 animate-fade-in-up">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <select
                          className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#e49d04] transition-all"
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="Entry Level (0-2 Yrs)">
                            Entry Level (0-2 Yrs)
                          </option>
                          <option value="Mid Level (3-5 Yrs)">
                            Mid Level (3-5 Yrs)
                          </option>
                          <option value="Senior Level (5+ Yrs)">
                            Senior Level (5+ Yrs)
                          </option>
                        </select>
                      </div>
                      <div className="pt-2 flex gap-3">
                        ...
                        <button
                          onClick={() => setIsFilterOpen(false)}
                          className="flex-1 py-2.5 bg-[#e49d04] text-[#0B0C10] font-bold text-sm rounded-xl hover:bg-[#cc8c03] transition-colors shadow-md shadow-[#e49d04]/10"
                        >
                          Apply
                        </button>
                      </div>
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                <button
                  ...
                  className="p-2.5 bg-white border border-gray-200 rounded-full text-gray-400 hover:text-[#0B0C10] hover:border-slate-200 hover:shadow-md transition-all relative"
                >
                  <FaBell size={18} />
                  {unreadCount > 0 && (
                    <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-[#EF4444] rounded-full border-2 border-white flex items-center justify-center text-white text-[8px] font-bold"></span>
                  )}
                </button>
                {isNotificationOpen && (
                  <>
                      ...
                      <div className="max-h-96 overflow-y-auto custom-scrollbar hide-scrollbar">
                        {notifications.length > 0 ? (
                          notifications.map((n: any) => (
                            <div
                              key={n._id}
                              className={`px-4 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors ${!n.isRead ? "font-semibold" : "text-gray-600"}`}
                            >
        ...
        <div className="p-6 lg:p-10 flex-1">
            ...
            <div className="space-y-8 animate-fade-in-up">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard
                  ...
                  label="Profile Views"
                  value={
                    isLoadingFreshUser ? "..." : profileViews.toLocaleString()
                  }
                  color="bg-slate-100 text-slate-600"
                />
              </div>
                ...
                <div className="xl:col-span-2 space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="text-sm font-bold text-[#0B0C10] hover:text-[#cc8c03]"
                    >
                      View All
                    </button>
                  </div>
          ...
          {activeTab === "explore" && (
            <div className="space-y-6 animate-fade-in-up">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Discover opportunities by category or interactive map
                  </p>
                </div>
                <div className="flex bg-gray-100 p-1 rounded-xl w-full sm:w-auto shrink-0">
                  <button
                    onClick={() => setExploreViewMode("map")}
                    className={`flex-1 sm:flex-none px-6 py-2.5 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${exploreViewMode === "map" ? "bg-[#0B0C10] shadow-sm text-[#e49d04]" : "text-gray-500 hover:text-[#121212]"}`}
                  >
                    <FaMap /> Map View
                  </button>
                  <button
                    onClick={() => setExploreViewMode("list")}
                    className={`flex-1 sm:flex-none px-6 py-2.5 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${exploreViewMode === "list" ? "bg-white shadow-sm text-[#0B0C10]" : "text-gray-500 hover:text-[#121212]"}`}
                  >
                    <FaList /> List View
                  </button>
                </div>
              ...
              <div
                className="category-carousel-container flex flex-col gap-4 relative w-full overflow-hidden py-2"
                style={{
                  maskImage:
                  ...
                  {[...Array(4)].map((_, arrayIndex) => (
                    <div key={arrayIndex} className="flex gap-4 shrink-0">
                      {[
                        ...
                        {
                          name: "Computer / Data Entry",
                          count: "309 openings",
                          icon: <FaFileAlt />,
                        },
                        {
                          name: "Hospitality/ Hotel/ Event",
                          count: "301 openings",
                          icon: <FaUsers />,
                        },
                        {
                        ...
                        <div
                          ...
                          className="flex items-center p-3 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-[#e49d04]/50 transition-all duration-300 cursor-pointer group w-72 shrink-0"
                        >
                          ...
                          <div className="ml-3 flex-grow min-w-0 text-left">
                            <h3 className="font-bold text-[#1a1a1a] text-[14px] leading-tight truncate">
                              {cat.name}
                            </h3>
              ...
              {exploreViewMode === "list" ? (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-6">
                  {isLoadingJobs ? (
                    <p className="text-sm text-gray-500 animate-pulse col-span-full">
                      Loading jobs...
                    </p>
                  ) : filteredJobs.length > 0 ? (
          ...
          {activeTab === "saved" && (
            <div className="space-y-6 animate-fade-in-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-6">
                {allJobs.filter((job: any) => savedJobIds.includes(job._id))
                  .length > 0 ? (
                  allJobs
                    .filter((job: any) => savedJobIds.includes(job._id))
                    .map((job: any) => (
                      <RecommendedJobCard
                        key={job._id}
                        job={job}
                  ...
                  <div className="col-span-full py-16 flex flex-col items-center justify-center bg-white rounded-3xl border border-dashed border-gray-200">
                    ...
                    <button
                      ...
                      className="mt-4 px-6 py-2 bg-[#e49d04] text-[#0B0C10] text-sm font-bold rounded-xl hover:bg-[#cc8c03] transition-colors shadow-lg shadow-[#e49d04]/20"
                    >
                      Explore Jobs
                    </button>
                  </div>
          ...
          {activeTab === "profile" && (
            <CandidateProfile user={user} setUser={setUser} />
          )}
          {activeTab === "messages" && (
            <CandidateMessagesSection
              user={user}
              allJobs={allJobs}
              initialSelectedUser={selectedMessageUser}
            />
          )}
                ...
                {appliedJobs.length > 0 ? (
                      ...
                      return filtered.map((job: any) => {
                        ...
                        return (
                          <ApplicationRow
                            ...
                            isVerifiedEmployer={
                              job.employerId?.gstVerificationStatus ===
                              "approved"
                            }
                            logo={(
                              job.employerId?.companyName ||
                  ...
                  <div className="text-center py-16">
                    ...
                    <button
                      ...
                      className="mt-6 px-6 py-2.5 bg-[#e49d04] text-[#0B0C10] font-bold rounded-xl shadow-lg hover:bg-[#cc8c03] transition-all"
                    >
                      Explore Jobs
                    </button>
                  </div>
...
const SettingsSection = ({ user }: { user: any }) => {
  ...
  const [requestDeleteOtp, { isLoading: isRequestingOtp }] =
    useRequestDeleteOtpMutation();
      ...
      <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
        ...
        <div className="border-2 border-red-200 border-dashed rounded-2xl p-6">
          ...
          <p className="text-sm text-gray-600 mt-2 mb-4">
            Once you delete your account, you will be logged out and will not be
            able to log in again. This action is irreversible.
          </p>
          <button
            ...
            {!otpSent ? (
              <>
                <p className="text-sm text-gray-500 mb-6">
                  This action cannot be undone. We will send an OTP to your
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 bg-red-500 text-white font-bold rounded-xl hover:bg-red-600 transition-colors shadow-lg disabled:opacity-50"
                  >
              ...
              <>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={isDeleting}
                    className="flex-1 py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition-colors shadow-lg disabled:opacity-50"
                  >
  ...
  const timerComponents = Object.keys(timeLeft).map((interval) => {
    ...
    return (
      <div key={interval} className="text-center">
        <div className="text-lg sm:text-xl font-bold text-[#0B0C10]">
          {timeLeft[interval as keyof typeof timeLeft]}
        </div>
        <div className="text-[9px] sm:text-[10px] uppercase font-bold text-gray-500">
          {interval}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex items-center gap-3 sm:gap-4">
      {timerComponents.length ? (
        ...
        <span className="text-sm font-bold text-red-500">Time's up!</span>
      )}
    </div>
  );
};
...
const ApplicationRow = ({
  ...
  return (
    <div className="flex flex-col p-4 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow group">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center text-xl font-bold text-[#0B0C10] group-hover:bg-[#0B0C10] group-hover:text-[#e49d04] transition-colors">
            {logo}
          </div>
          <div>
            <h4 className="font-bold text-[#121212] text-sm md:text-base group-hover:text-[#cc8c03] transition-colors">
              {title}
            </h4>
        ...
        <div className="flex items-center gap-4 md:gap-6">
          <div className="hidden sm:block text-right">
            <span className="text-[10px] font-bold text-gray-400 block mb-0.5 uppercase tracking-wider">
              Applied
            </span>
            <span className="text-xs font-medium text-[#121212]">{date}</span>
          </div>
          <div
            className={`px-2.5 py-1.5 md:px-3 md:py-1.5 rounded-lg border text-[10px] md:text-xs font-bold flex items-center gap-1.5 ${statusConfig.color}`}
          >
            {statusConfig.icon}{" "}
      ...
      {status === "Interview" && detail?.interviewDate && (
        <div className="mt-4 pt-4 border-t border-gray-50 flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-[#0B0C10]/5 p-4 rounded-xl border border-gray-200 gap-3">
            <div>
              <p className="text-xs font-bold text-[#0B0C10] uppercase tracking-wider mb-1">
                Interview Scheduled
              </p>
              <p className="text-sm font-medium text-gray-700">
                {new Date(detail.interviewDate).toLocaleString("en-US", {
                  year: "numeric",
                  month: "short",
                  day: "numeric",
                  hour: "numeric",
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-sm">
            <p className="mb-2">
              <span className="font-bold text-[#0B0C10]">Meeting Link:</span>{" "}
              <a
            ...
            <p>
              <span className="font-bold text-[#0B0C10]">Instructions:</span>{" "}
              <span className="text-gray-600">
                {detail.interviewDescription || "None provided."}
              </span>
            </p>
          </div>
        </div>
...
const RecommendedJobCard = ({
  ...
  const company = job
    ? employerId?.companyName || employerId?.name || "Company"
    : propCompany;
  ...
  const verified = job
    ? employerId?.gstVerificationStatus === "approved"
    : isVerifiedEmployer;
  const location = job?.location || propLocation;
  const handleCompanyClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (employerId?._id && onViewCompany) onViewCompany(employerId._id);
  };
  return (
    <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow group flex flex-col">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 bg-slate-50 rounded-lg flex items-center justify-center text-lg font-bold text-[#0B0C10] flex-shrink-0">
            {logo}
          </div>
          <div className="min-w-0">
            <h4 className="font-bold text-[#121212] text-sm group-hover:text-[#0B0C10] transition-colors truncate">
              {title}
            </h4>
            <span
              onClick={handleCompanyClick}
              className="text-xs font-medium text-gray-500 hover:underline hover:text-[#e49d04] transition-colors truncate block cursor-pointer"
            >
              {company}
            </span>
          </div>
        ...
        <button
          ...
          className={`transition-colors flex-shrink-0 ml-2 ${isSaved ? "text-amber-500 hover:text-amber-600" : "text-gray-300 hover:text-[#0B0C10]"}`}
        >
          <FaBookmark />
        </button>
      </div>
      <div className="flex flex-col gap-2 mb-4">
        ...
        <div className="flex items-center text-xs text-gray-500">
          <FaMoneyBillWave className="mr-2 flex-shrink-0" /> {salary}
        </div>
      </div>
      <div className="flex flex-wrap gap-2 mb-4">
        {tags.map((tag: string, i: number) => (
          <span
            key={i}
            className="text-[10px] font-bold px-2 py-1 bg-slate-50 text-slate-600 rounded-md border border-slate-100"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="mt-auto">
        <button
          ...
          className="w-full py-2.5 bg-slate-50 text-[#0B0C10] font-bold text-sm rounded-xl hover:bg-[#0B0C10] hover:text-[#e49d04] transition-colors border border-slate-100 group-hover:border-[#0B0C10]"
        >
          View Job Details
        </button>
      </div>
      ...
      >
        ...
        <div className="p-6 border-t border-gray-100 flex items-center justify-end bg-gray-50/50 rounded-b-3xl">
          <button
            onClick={handleApply}
            disabled={isApplying}
            className="px-8 py-3 bg-[#e49d04] text-[#0B0C10] font-black rounded-xl hover:bg-[#cc8c03] shadow-lg shadow-[#e49d04]/20 transition-all transform hover:-translate-y-0.5 flex items-center gap-2 text-sm disabled:opacity-50"
          >
...
const ResumeUploadModal = ({ user, setUser, jobToApply, onClose }: any) => {
  ...
  const handleUploadAndApply = async () => {
    ...
    try {
      ...
      await applyForJob({
        ...
        candidateId: updatedUser._id,
      }).unwrap();
      toast.success(
        `🎉 Successfully applied for ${jobToApply.title}! The employer has been notified.`,
        {
          duration: 5000,
          position: "top-center",
          style: {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-3xl shadow-2xl w-full max-w-md relative animate-fade-in-up"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6 border-b border-gray-100">
          ...
          <p className="text-sm text-gray-500 mt-1">
            You need to upload a resume before applying for "{jobToApply?.title}
            ".
          </p>
        </div>
        <div className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              accept=".pdf,.doc,.docx"
              className="mt-2 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-yellow-50 file:text-yellow-700 hover:file:bg-yellow-100 focus:outline-none"
            />
          </div>
        </div>
        <div className="p-6 bg-gray-50/50 border-t border-gray-100 flex justify-end gap-3">
          ...
          <button
            ...
            className="px-5 py-2.5 text-sm font-bold text-[#0B0C10] bg-[#e49d04] rounded-xl hover:bg-[#cc8c03] shadow-md disabled:opacity-50"
          >
            {isUploading
              ? "Uploading..."
              : isApplying
...
const CompanyProfileModal = ({
  ...
  const { data: company, isLoading: isLoadingCompany } = useGetCompanyByIdQuery(
    companyId,
    { skip: !companyId },
  );
  const { data: jobs = [], isLoading: isLoadingJobs } =
    useGetJobsByEmployerQuery(companyId, { skip: !companyId });
  const [toggleFollow, { isLoading: isFollowingLoading }] =
    useToggleFollowCompanyMutation();
  ...
  const handleFollowToggle = async () => {
    try {
      const { result: updatedCandidate } = await toggleFollow({
        companyId,
      ...
      toast.success(
        isFollowing
          ? `Unfollowed ${company.companyName}`
          : `Now following ${company.companyName}`,
      );
    } catch (error) {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="mt-4 px-4 py-2 bg-gray-200 font-bold text-sm rounded-xl"
          >
            Close
          </button>
        </div>
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl relative animate-fade-in-up overflow-hidden flex flex-col max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="overflow-y-auto custom-scrollbar flex-1 bg-gray-50 pb-8">
          <div
            className="h-48 md:h-64 bg-gradient-to-r from-[#0B0C10] to-[#1F2833] relative bg-cover bg-center flex-shrink-0"
            style={{
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 bg-black/20 hover:bg-black/40 text-white p-2.5 rounded-full backdrop-blur-md transition-colors z-10"
            >
              <FaTimes size={18} />
            </button>
          </div>
              ...
              <div className="flex flex-col md:flex-row gap-6 items-start justify-between">
                <div className="flex flex-col md:flex-row gap-6 items-start flex-1">
                  <div className="w-24 h-24 md:w-28 md:h-28 bg-white rounded-2xl p-1.5 shadow-lg -mt-16 flex-shrink-0 border border-gray-100">
                    <div className="w-full h-full bg-slate-50 rounded-xl flex items-center justify-center text-4xl font-bold text-[#0B0C10] overflow-hidden">
                  ...
                  <div className="flex-1 mt-2 md:mt-0">
                    <h1 className="text-2xl md:text-3xl font-bold text-[#121212] flex items-center flex-wrap gap-3">
                      ...
                      {company.gstVerificationStatus === "approved" && (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-green-50 text-green-700 text-xs font-black uppercase tracking-wider border border-green-200 shadow-sm">
                          <FaCheckCircle size={14} /> Verified
                        </span>
                      )}
                    </h1>
                    <p className="text-gray-500 text-sm md:text-base mt-1 font-medium">
                      {company.tagline || "Leading the way in innovation."}
                    </p>
                    <div className="flex flex-wrap items-center gap-3 mt-4 text-xs font-medium text-gray-500">
                      {company.location && (
                        <span className="flex items-center gap-1.5 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                          <FaMapMarkerAlt className="text-[#0B0C10]" />{" "}
                          {company.location}
                        </span>
                      )}
                      {company.industry && (
                        <span className="flex items-center gap-1.5 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                          <FaBuilding className="text-[#0B0C10]" />{" "}
                          {company.industry}
                        </span>
                      )}
                      {company.companySize && (
                        <span className="flex items-center gap-1.5 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                          <FaUsers className="text-[#0B0C10]" />{" "}
                          {company.companySize}
                        </span>
                      )}
                      {company.website && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex items-center gap-1.5 text-[#0B0C10] hover:underline bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100"
                        >
                          <FaGlobe /> Website
                        </a>
                      )}
                      <span className="flex items-center gap-1.5 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                        <FaEye className="text-[#0B0C10]" />{" "}
                        {company.profileViews || 0} Views
                      </span>
                    </div>
                  </div>
                </div>
                <div className="mt-4 md:mt-0 flex-shrink-0 w-full md:w-auto flex flex-col sm:flex-row gap-3">
                  <button
                    ...
                    className={`w-full md:w-auto px-6 py-2.5 rounded-xl font-bold transition-all flex items-center justify-center gap-2 text-sm disabled:opacity-70 ${isFollowing ? "bg-gray-100 text-gray-600 border border-gray-200" : "bg-[#e49d04] text-[#0B0C10] shadow-lg shadow-[#e49d04]/20"}`}
                  >
                    {isFollowingLoading ? (
                  ...
                  {isFollowing && (
                    <button
                      onClick={() => onMessageClick(company)}
                      className="w-full md:w-auto px-6 py-2.5 rounded-xl font-bold transition-all flex items-center justify-center gap-2 text-sm bg-[#0B0C10] text-[#e49d04] hover:bg-[#1F2833] border border-transparent"
                    >
                  ...
                  {isLoadingJobs ? (
                    ...
                    <div className="space-y-4">
                      {jobs.map((job: any) => (
                        <div
                          key={job._id}
                          className="bg-gray-50 rounded-2xl p-5 border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4"
                        >
                          <div>
                            <h3 className="font-bold text-[#121212]">
                              {job.title}
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                              {job.location} • {job.workMode} • ${job.salaryMin}
                              -${job.salaryMax}
                            </p>
                          </div>
                          <button
                            ...
                            className="px-5 py-2 bg-white border border-gray-200 text-[#0B0C10] text-sm font-bold rounded-xl hover:bg-[#e49d04] hover:border-[#e49d04] shadow-sm transition-colors whitespace-nowrap"
                          >
                            View Details
                          </button>
                        </div>
...
const CandidateMessagesSection = ({
  user,
  allJobs,
  initialSelectedUser,
}: any) => {
  ...
  if (user?.followingCompanies && Array.isArray(user.followingCompanies)) {
    allJobs.forEach((job: any) => {
      if (
        job.employerId &&
        user.followingCompanies.includes(job.employerId._id)
      ) {
  ...
  if (
    initialSelectedUser &&
    !followedCompaniesMap.has(initialSelectedUser._id)
  ) {
  ...
  const filteredCompanies = followedCompanies.filter(
    (c: any) =>
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm flex h-[650px] overflow-hidden animate-fade-in-up relative">
      ...
      <div
        className={`w-full md:w-80 border-r border-gray-100 flex-col bg-white z-10 flex-shrink-0 ${selectedUser ? "hidden md:flex" : "flex"}`}
      >
      ...
      {/* Chat Area */}
      <div
        className={`flex-1 flex-col bg-[#F8FAFC] ${!selectedUser ? "hidden md:flex" : "flex"}`}
      >
        {selectedUser ? (
          <>
            {/* Chat Header */}
            <div className="px-6 py-4 border-b border-gray-200 bg-white flex items-center gap-4 shadow-sm z-10">
              <button
                ...
                className="md:hidden text-gray-500 hover:text-[#0B0C10] p-2 -ml-2 transition-colors"
              >
              ...
              <div>
                ...
                <p className="text-xs text-green-600 font-medium mt-0.5 flex items-center gap-1.5">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
              {messages.map((msg: any) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`px-5 py-3 rounded-2xl max-w-[85%] text-sm shadow-sm ${isMine ? "bg-[#0B0C10] text-white rounded-br-sm" : "bg-white border border-gray-200 text-gray-800 rounded-bl-sm"}`}
                    >
            ...
            <div className="p-4 bg-white border-t border-gray-200 flex gap-3 items-center shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
              ...
              <button
                onClick={handleSend}
                disabled={isSending}
                className="w-12 h-12 rounded-full flex items-center justify-center bg-[#e49d04] text-[#0B0C10] font-bold hover:bg-[#cc8c03] shadow-md shadow-[#e49d04]/20 transition-all transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
              >
                {isSending ? (
                  <FaSpinner className="animate-spin" />
                ) : (
...
const JobAlertsSection = ({ user }: { user: any }) => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!keyword && !location)
      return toast.error("Please provide a keyword or location");
    setIsCreating(true);
    try {
        ...
        >
          <div className="w-full md:flex-1">
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#e49d04] text-sm"
            />
          </div>
          <div className="w-full md:flex-1">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-2">
          ...
          <button
            type="submit"
            disabled={isCreating}
            className="w-full md:w-auto px-8 py-3 bg-[#e49d04] text-[#0B0C10] font-bold rounded-xl hover:bg-[#cc8c03] transition-colors shadow-md disabled:opacity-50 h-[46px]"
          >
            {isCreating ? "Saving..." : "Create Alert"}
          </button>
        </form>
      </div>
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm">
        ...
        {isLoading ? (
          <div className="text-center py-8">
            <FaSpinner className="animate-spin text-2xl text-gray-300 mx-auto" />
          </div>
        ) : alerts.length > 0 ? (
          <div className="space-y-4">
            {alerts.map((alert) => (
              ...
              >
                <div>
                  <h4 className="font-bold text-[#121212]">
                    {alert.keyword || "Any Role"}
                  </h4>
                  <p className="text-sm text-gray-500 flex items-center gap-1 mt-1">
                    <FaMapMarkerAlt className="text-[#e49d04]" />{" "}
                    {alert.location || "Any Location"}
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  <span className="hidden sm:inline-block px-3 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-lg">
                    Active (Daily)
                  </span>
                  <button
          ...
          <div className="text-center py-12 border-2 border-dashed border-gray-100 rounded-2xl">
            ...
            <p className="text-gray-500 font-medium">
              You don't have any job alerts set up yet.
            </p>
          </div>
        )}
      </div>
...
function useIncrementProfileViewMutation(): [any] {
  const incrementProfileView = async (params: {
    id: string;
    viewerId: string;
  }) => {
    try {
      const response = await fetch(
        `/api/companies/${params.id}/increment-view`,
        {
          method: "POST",
          headers: {
...
const InteractiveJobMap = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setMapData({ RL, customIcon, MapController });
      } catch (err) {
  ...
  const pins = [
    { name: "Amritsar", pos: [31.634, 74.8723], jobs: 123, state: "Punjab" },
    { name: "Jalandhar", pos: [31.326, 75.5762], jobs: 43, state: "Punjab" },
    { name: "Ludhiana", pos: [30.901, 75.8573], jobs: 71, state: "Punjab" },
    ...
    { name: "New Delhi", pos: [28.6139, 77.209], jobs: 850, state: "Delhi" },
  ];
  ...
  const visiblePins =
    selectedState === "India"
      ? pins
      : pins.filter((p: any) => p.state === selectedState);

  if (!mapData) {
    return (
      <div className="w-full h-[600px] bg-gray-100 border border-gray-200 rounded-3xl animate-pulse flex items-center justify-center text-gray-400 font-bold mt-6 shadow-sm">
  ...
  const {
    RL: { MapContainer, TileLayer, Marker, Popup },
    customIcon,
    MapController,
  } = mapData;
    ...
    <div className="relative w-full h-[600px] rounded-3xl overflow-hidden shadow-sm border border-gray-200 mt-6 z-0">
      <div className="absolute top-6 right-6 z-[1000] bg-white p-4 rounded-2xl shadow-lg border border-gray-100">
        ...
        <select
          ...
          className="w-full bg-gray-50 text-[#121212] border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#e49d04] cursor-pointer"
        >
      ...
      >
        ...
        {visiblePins.map((pin: any, idx: number) => (
          <Marker
            key={idx}
            position={pin.pos as [number, number]}
            icon={customIcon}
          >
            <Popup className="custom-popup rounded-2xl">
              <div className="text-center p-1 min-w-[120px]">
                <h4 className="font-bold text-[#121212] text-base">
                  {pin.name}
                </h4>
                <div className="bg-yellow-50 text-yellow-700 border border-yellow-200 px-3 py-1 rounded-lg text-xs font-bold my-2">
                  {pin.jobs} Active Jobs
                </div>
                <button
                  onClick={() => onSelectCity(pin.name)}
                  className="w-full bg-[#121212] text-white text-xs font-bold py-2 rounded-xl hover:bg-gray-800 transition-colors shadow-md"
                >
                  View Jobs
                </button>
              </div>
