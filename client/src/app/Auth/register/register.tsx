...
const RegisterModal: React.FC<RegisterModalProps> = ({ onClose }) => {
  ...
  const btnDisabled = "w-full flex justify-center py-3.5 px-4 border border-transparent rounded-xl shadow-none text-sm font-bold text-white bg-gray-300 cursor-not-allowed transition-all duration-200";
    ...
    <>
      ...
      <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); handleNext(); }}>
        ...
        <div>
          ...
          <input id="email-register" name="email" type="email" autoComplete="email" placeholder="john@example.com" required className={inputClass} />
        </div>
        <div>
          <label htmlFor="password-register" className={labelClass}>Password</label>
          <input id="password-register" name="password" type="password" placeholder="••••••••" required className={inputClass} />
        </div>
        <div>
          <label htmlFor="confirm-password" className={labelClass}>Confirm Password</label>
          <input id="confirm-password" name="confirm-password" type="password" placeholder="••••••••" required className={inputClass} />
        </div>
        <div className="pt-4">
  ...
  const renderStep2 = () => (
    <>
      ...
      <h2 className="text-2xl font-bold text-center mb-2 text-gray-900">Join as a Job Seeker or Employer</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div
          ...
          className="p-6 border-2 border-gray-100 rounded-2xl text-center cursor-pointer hover:border-blue-600 hover:bg-blue-50/50 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 group"
        >
          <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-blue-600 transition-colors duration-300"><FaUser className="text-2xl text-blue-600 group-hover:text-white transition-colors duration-300" /></div>
          <h3 className="font-bold text-lg text-gray-900 mb-2">I'm a Job Seeker</h3>
        ...
        <div
          ...
          className="p-6 border-2 border-gray-100 rounded-2xl text-center cursor-pointer hover:border-blue-600 hover:bg-blue-50/50 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 group"
        >
          <div className="w-16 h-16 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-purple-600 transition-colors duration-300"><FaBuilding className="text-2xl text-purple-600 group-hover:text-white transition-colors duration-300" /></div>
          <h3 className="font-bold text-lg text-gray-900 mb-2">I'm an Employer</h3>
  ...
  const renderSeekerStep3 = () => (
    <>
      ...
      <h2 className="text-2xl font-bold text-center mb-6 text-gray-900">Basic Information</h2>
      <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); handleNext(); }}>
        <div>
          <label htmlFor="headline" className={labelClass}>Professional Headline</label>
          <input id="headline" name="headline" type="text" placeholder="e.g., Senior Software Engineer" required className={inputClass} />
        </div>
        <div>
          <label htmlFor="location" className={labelClass}>Location</label>
          <input id="location" name="location" type="text" placeholder="e.g., San Francisco, CA" required className={inputClass} />
        </div>
        <div>
  ...
  const renderSeekerStep4 = () => (
    <>
      ...
      <h2 className="text-2xl font-bold text-center mb-6 text-gray-900">Experience & Education</h2>
      <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); handleNext(); }}>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input id="education" name="education" type="text" placeholder="Highest Degree" className={inputClass} />
          </div>
        </div>
        <div className="pt-4">
          <button type="submit" className={btnPrimary}>
  ...
  const renderSeekerStep5 = () => (
    <>
      ...
      <h2 className="text-2xl font-bold text-center mb-6 text-gray-900">Skills & Resume</h2>
      <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); if (isRobotChecked) onClose(); /* Final step */ }}>
        <div>
          ...
          <input id="skills" name="skills" type="text" placeholder="e.g. React, Node.js, Design" className={inputClass} />
        </div>
        <div>
          <label htmlFor="resume" className={labelClass}>Upload your Resume</label>
          <div className="mt-1 flex justify-center px-6 pt-8 pb-8 border-2 border-gray-200 border-dashed rounded-xl hover:border-blue-500 hover:bg-blue-50 transition-all duration-300 cursor-pointer group">
        ...
        <div className="flex items-center p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-blue-300 transition-colors">
          <input
        ...
        <div className="pt-4">
          <button type="submit" disabled={!isRobotChecked} className={isRobotChecked ? btnPrimary : btnDisabled}>
  ...
  const renderEmployerStep3 = () => (
    <>
      ...
      <h2 className="text-2xl font-bold text-center mb-6 text-gray-900">Tell us about your company</h2>
      <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); if (isRobotChecked) onClose(); /* Final step */ }}>
        <div>
          <label htmlFor="company-name" className={labelClass}>Company Name</label>
          <input id="company-name" name="company-name" type="text" placeholder="e.g., TechCorp Inc." required className={inputClass} />
        </div>
        <div>
          <label htmlFor="company-size" className={labelClass}>Company Size</label>
          <select id="company-size" name="company-size" required className={inputClass}>
        ...
        <div>
          ...
          <input id="phone-employer" name="phone" type="tel" placeholder="+1 (555) 000-0000" className={inputClass} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="industry" className={labelClass}>Industry</label>
            <input id="industry" name="industry" type="text" placeholder="e.g. Technology" className={inputClass} />
          </div>
          <div>
            <label htmlFor="website" className={labelClass}>Website</label>
            <input id="website" name="website" type="url" placeholder="https://example.com" className={inputClass} />
          </div>
        </div>
        <div>
          <label htmlFor="your-role" className={labelClass}>Your Role</label>
          <input id="your-role" name="your-role" type="text" placeholder="e.g., Hiring Manager" required className={inputClass} />
        </div>
        <div>
          <label htmlFor="description" className={labelClass}>Company Description</label>
          <textarea id="description" name="description" rows={3} placeholder="Briefly describe your company..." className={inputClass}></textarea>
        </div>
        <div className="flex items-center p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-blue-300 transition-colors">
          <input
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4 transition-all duration-300">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative animate-fade-in-up overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="relative h-1.5 bg-gray-100 w-full">
           <div className="absolute top-0 left-0 h-full bg-blue-600 transition-all duration-500 ease-out" style={{ width: `${progress}%` }}></div>
        </div>
